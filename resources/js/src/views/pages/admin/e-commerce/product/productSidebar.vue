              ...
              <vs-tabs>
                ...
                <vs-tab :label="$t('ImageArea')">
                  ...
                  <div class="mt-8 mb-base">
                    ...
                    <span>{{$t('You_can_upload_your_gallery_to_here')}}</span>

                    <div  v-for="(file,index) in fileList" :key="index" class="vx-col w-full sm:w-1/5 lg:w-1/5 mb-base">
                        <vx-card >
                            <div slot="no-body">
                            <img height="100px" width="100px" :src="file"   class="responsive" >
                            </div>
                            <div class="flex justify-between flex-wrap">
                            <vs-button color="danger" type="filled" icon-pack="feather" @click="deleteData(index)" icon="icon-trash"></vs-button>
                            {{$t('deleteIconDirectDeleting')}}
                            </div>
                        </vx-card>
                     </div>
...
export default {
  ...
  watch: {
    isSidebarActive (val) {
      ...
      if (this.data.newData) {
        ...
        this.price = price
        this.fileList = JSON.parse(options).gallery
  ...
  methods: {
    ...
    showAlert (title, text, icon, color) {
      ...
      this.$store.dispatch('post/removeProductGalleryImage', obj)
            .then((response) => {
                    console.log(response.data)
            this.fileList = response.data.gallery
              this.$vs.notify({
                title: 'Başarılı',
                text: 'içerik silindi',
                iconPack: 'feather',
